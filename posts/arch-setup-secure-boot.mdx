...
本人用的RedmiBook Pro 15S，也就是红米锐龙版，是在关闭了安全启动的情况下进行的操作
如果你用的是联想电脑，发现密钥写不进去的，请参考官方教程，将安全启动模式设置成“设置模式（Setup mode）”，然后启动进入arch。
...
完成上面的操作后，重启到BIOS，开启安全启动，然后试试能不能进入系统
